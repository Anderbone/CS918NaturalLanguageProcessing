...
def get_vect():
    if os.path.isfile('X.pkl') and os.path.isfile('vector.pkl'):
        ...
        vect = CountVectorizer(stop_words='english',ngram_range=(1,2), max_features=15000, lowercase=True)
...
def senti_score(split_corpus):
    data_list = []
    # lemmatizer = WordNetLemmatizer()
    after_tag = nltk.pos_tag(split_corpus)
    print(after_tag)
    pos_feature_line = []
    for word, tag in after_tag:
        if tag.startswith('N'):
...
for classifier in ['Logistic Regression','Random Forest', 'SVM','Naive Bayes','KNN']:
